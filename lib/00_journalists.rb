def create_handles 
    ...
    # short = trie mon array 
    # short_by trie pas  ma taille 
    #Combien y a-t-il de handle dans cette array ?
def count_handle(handles)
	puts "Il y a #{handles.count} handles dans l'array."
end

    #Quel est le handle le plus court de cette liste ?
def min_handles(handles)
	puts ("le handle le plus court de cette liste est #{handles.sort_by(&:length)[0]}")
end

    #Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)
def caract5_handles(handles)
   	puts "le nombre de hand"
    puts handles.count{|a| a.length == 6}
end 

    #Trie la liste de handle par ordre alphabétique.
def order_handles(handles)   
	puts "voici la listes des handles par ordres alphabétique  "  
	puts handles.sort_by {|a| a.downcase}
end 

	
	#Trie la liste de handle par taille des handle 
def size_handle(handles)
 ...
 def find_handle(handles)
    ...
    puts handles.find_index("@epenser")+1
 end

  #(nombre de handle avec 1 caractère, nombre de handle avec 2 caractères, nombre de handle avec 3 caractères, etc)

 def size_count(handles)
 	puts "Sors-moi une répartition des handle par taille de ces derniers"
    i = 0
    handles.max{|a, b| a.length <=> b.length} #combien de caractere max il y a (33)
    while (i <= 33)
    print "nombre d'handle avec #{i} caractere : "
    puts handles.count{|a| a.length == i}
    i += 1
    end
end
...
def perform
	...
	puts " test"
	count_handle(handles)
	puts " test2"
	min_handles(handles)
	puts " test3"
	caract5_handles(handles)
	puts " test4"

	order_handles(handles)
	puts " test 5"
	size_handle(handles)
	puts "test 6"
	find_handle(handles)
